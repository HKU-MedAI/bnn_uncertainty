...
from tests_detectron.detectron.detectron import infer_labels
from data import sample_data
from data.core import split_dataset
import argparse
import time

from models import AlexNet, LeNet, CNN, ResNet

parser = argparse.ArgumentParser()
parser.add_argument('--run_name', type=str, default="mmnist_fashionmnist_lenet",
                    help='Name of the run, data will be stored in results/args.run_name')
...
parser.add_argument('--resume', default=True, action='store_true', help='If not given and run_name exists, will error')
...
#                                     verbose=False)
p_train, p_val, p_test_all = sample_data.mnist(split='all')
q_all = sample_data.fashion_mnist()

test_sets = {'p': p_test_all, 'q': q_all}
# base_model = AlexNet(inputs=3, outputs=10)
    ...
    for dataset_name in args.splits:
        for seed in range(args.seeds):
            ...
            pl.Trainer(accelerator='gpu',
                       devices=gpus, logger=False, max_epochs=1).test(base, pq_loader.test_dataloader(), verbose=False)
