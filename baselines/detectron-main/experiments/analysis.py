...
val_p = [torch.load(x) for x in (glob(f'{run_name}/val_*_p_*.pt'))]
...
for N in Ns:
    ...
    thresh = np.where(fpr_mean <= alpha)[0][-1]

    tpr_sem_at_5 = sem(tpr[thresh])
    # tpr = tpr.mean(1)
    tpr_mean_at_5 = tpr.mean(1)[thresh]

    # Compute AUROC
    aurocs = []
    for i in range(tpr.shape[1]):
        auc = skm.roc_auc_score(tpr[:, i], fpr[:, i])
        aurocs.append(auc)
    ...
    auprs = []
    for i in range(tpr.shape[1]):
        precision, recall, thresholds = precision_recall_curve([0, 1], [alt[i], null[i]])
        aupr = skm.auc(recall, precision)
        auprs.append(aupr)
    aupr = np.mean(auprs)
    aupr_sd = np.std(auprs)

    print(f'TPR: {tpr_mean_at_5:.2f} \u00B1 {tpr_sem_at_5:.2f}'.replace('0.', '.'),
          f'AUROC: {auroc:.4f} \u00B1 {auroc_sd:.3f}', f'AUPR: {aupr:.4f} \u00B1 {aupr_sd:.3f}', sep=' ',
          end=' | ' if N != Ns[-1] else '')
print()
