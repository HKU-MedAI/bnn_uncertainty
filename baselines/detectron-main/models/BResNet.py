...
class BBBResNet(nn.Module):
    def __init__(self, outputs, inputs, image_size=256, priors=None, activation_type='softplus', get_sig=False):
        ...
        self.fc = BBBLinear(1000, 16, bias=True, priors=self.priors)
        self.classifier = BBBLinear(16, outputs, bias=True, priors=self.priors)

        self.is_de = get_sig
        if get_sig:
            self.fc_sig = BBBLinear(1000, outputs, bias=True, priors=self.priors)
